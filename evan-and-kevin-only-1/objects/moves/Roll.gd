class_name RollMove extends BaseMove
...
func _init() -> void:
	...
	descr = "Increases user's Speed by 25%, or 50% if foe is weak to this move."
...
func on_hit(user: BaseCreature, target: BaseCreature, _battle_properties: Array, _talismans: Array) -> String:
	if target.weak_to_type(element_id):
		...
		user.speed *= 1.5
		return "[color=#fff700]" + user.display_name + "'s Speed increased by 50%![/color]"
	else:
		@warning_ignore("narrowing_conversion")
		user.speed *= 1.25
		return user.display_name + "'s Speed increased by 25%!"
